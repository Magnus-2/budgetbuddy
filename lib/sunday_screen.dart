...
class SundayScreen extends StatefulWidget {
  ...
  @override
  State<SundayScreen> createState() => SundayScreenState();
}

class SundayScreenState extends State<SundayScreen> {
  ...
  Future<void> _plusBudget(String inputtext) async {
    ...
    int? exSu = prefs.getInt('exSu') ?? 0;
    result = help + exSu;
    prefs.setInt('exSu', result);
    _exSu = _prefs.then((SharedPreferences prefs) {
      return prefs.getInt('exSu') ?? 0;
    });
    mainoutput();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Sunday Expenditures'),
        ),
        body: Container(
